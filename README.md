...
The machine learning molecular dynamics (mlmd) package is a command line open source python program. It calculates the feature representation of a structure (molecule, cluster, crystal), using the Structural Information Filter Features (SIFF), in addition,  mlmd is also able to perform molecular dynamics simulations with machine learning potentials (also known as machine learning force fields or machine learning empirical potentials). 
